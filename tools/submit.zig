...
pub fn main(init: std.process.Init) !u8 {
    ...
    const result = client.fetch(.{
        ...
        // 実測ではアクセスキー拒否 (401 相当) と、サーバー全面停止 (503) の両方でこうなる
        error.WriteFailed => {
            std.debug.print("送信中に接続が打ち切られました。アクセスキー拒否かサーバー停止 (503) の可能性があります。\n" ++
                "mise run leaderboard が失敗するならサーバー側の停止です。復旧を待ってから、\n" ++
                "mise run status で提出が作られていないことを確認して再提出してください。\n", .{});
            return 1;
        },
        else => return err,
    };
