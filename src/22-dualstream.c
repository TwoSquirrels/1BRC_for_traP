// 17-renumber の本走を 2 ストリーム/スレッドの行交互パースにした版 (08 の再審)。
// サイクル会計で「次の行頭 = 今の行の '\n' 位置」の直列連鎖 (≈ 13〜15 サイクル/リンク)
// が実測の未説明ストールと一致し、これが天井の疑いが濃厚なため、鎖を 2 本に割って
// out-of-order に重ねさせる。08 (4 本/スレッド = 計 32 本) はメモリアクセス形状の崩壊で
// 大惨事だったが、計 16 本なら免れる可能性に賭ける。手元では判定不能につき本番で決着。
...
// 担当範囲を前半・後半の 2 ストリームに割り、1 行ずつ交互にパースする。
// 「次の行頭 = 今の行の '\n' 位置」という直列連鎖 (1 リンク ≈ 13〜15 サイクル) が
// 2 本の独立した鎖に割れ、out-of-order 実行が互いのリンクを重ねられる。
// ストリームは計 2 本/スレッド (プロセス全体 16 本) に抑え、08 (32 本) の
// メモリアクセス形状崩壊を避ける。どちらかが尽きたら残りは単独ストリームで処理
static void run_dual(Ctx *c, const char *pa, const char *ea, const char *pb, const char *eb) {
    Row rows[BATCH];
    Stats *dst[BATCH];
    while (pa < ea && pb < eb) {
        int n = 0;
        while (n + 2 <= BATCH && pa < ea && pb < eb) {
            pa = parse_row(&rows[n], pa);
            __builtin_prefetch(&c->table[rows[n].slot], 0, 3);
            n++;
            pb = parse_row(&rows[n], pb);
            __builtin_prefetch(&c->table[rows[n].slot], 0, 3);
            n++;
        }
        for (int i = 0; i < n; i++) {
...
static void *worker(void *arg) {
    ...
    // 本走: 残りを行整列された前半・後半に割って 2 ストリームで流す
    const char *mid = sample_end + (c->end - sample_end) / 2;
    if (mid > sample_end && mid < c->end) {
        const char *nl = memchr(mid, '\n', (size_t)(c->end - mid));
        mid = nl != nullptr ? nl + 1 : c->end;
    } else {
        mid = c->end;
    }
    run_dual(c, sample_end, mid, mid, c->end);
    return nullptr;
}
