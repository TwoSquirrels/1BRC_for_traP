// 17-renumber の message_length パースを低乗算化した版。stamp_count で実証済みの手法の
// 写しで、2〜3 桁 (行数の 99.4%) を乗算なしの lea Horner + cmov で取り、4 桁以上だけ
// SWAR (乗算 3 回) に落とす。Port 1 (乗算ポート) 減圧の最後の残り。
...
HOT_INLINE const char *parse_row(Row *r, const char *p) {
    ...
    if (nl != 0) {  // 区切り込み 8 バイト以内 (実測 99% 超): よく当たる分岐
        ...
        // message_length は 2〜3 桁が 99.4% なので、3 桁までは乗算なし (lea の Horner + cmov 2 個)
        uint32_t b0 = (uint32_t)(w & 0x0F);
        uint32_t b1 = (uint32_t)((w >> 8) & 0x0F);
        uint32_t b2 = (uint32_t)((w >> 16) & 0x0F);
        uint32_t v2 = b0 * 10 + b1;
        uint32_t v3 = v2 * 10 + b2;
        r->len = k1 == 1 ? b0 : k1 == 2 ? v2 : v3;
        if (__builtin_expect(k1 > 3, 0)) {  // 4 桁以上だけ SWAR で引き直す
