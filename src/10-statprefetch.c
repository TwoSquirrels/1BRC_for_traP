// 09-numfast のバッチ処理を三段パイプラインにした集計先プリフェッチ版。09 からの差分は
// worker の後段構造のみ。perf 実測で「stats[id][month] の行ロード待ちが全体の約 26%」
// (プリフェッチは表スロットのみで集計先はノーガード) と判明したため、バッチを
//   (1) パース + 表スロットのプリフェッチ
//   (2) 照合で id を確定 + stats[id][month] のプリフェッチ (書き込み予定なので write ヒント)
//   (3) 集計
// の三段に分け、集計先ラインのレイテンシを段間で覆い隠す。データ依存の分岐は増やさず、
// シーケンシャル走査も壊さない (08 の教訓)。
