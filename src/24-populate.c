...
// MADV_POPULATE_READ (Linux 5.14+) は範囲のページテーブルを一括で埋める。musl のヘッダに
// 無いことがあるため番号を直接与える。ページキャッシュ温の入力なら I/O は起きず、毎行
// アクセス時の minor fault トラップ (26 GB = 数百万回) を syscall 一発に畳む狙い
...
static void *worker(void *arg) {
    Ctx *c = arg;
    // 担当範囲を先に一括 populate してから処理する。フォルトを本走ループから追い出す
    madvise((void *)c->begin, (size_t)(c->end - c->begin), MADV_POPULATE_READ);
