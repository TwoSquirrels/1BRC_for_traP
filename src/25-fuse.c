...
// 識別キーと 12 ヶ月分の集計を 1 スロットに融合する。旧設計の table[slot] -> id ->
// stats[id][month] という別配列 2 段のランダムアクセスを、スロット内の連続領域への
// 1 アクセスに畳む狙い。id 間接参照と renumber は融合と両立しないので落ちる。
typedef struct {
    uint64_t first8, last8;  // 名前の先頭/末尾 8 バイト (8 バイト未満はマスク済み)
    uint64_t mid8;           // 名前の [8, 16) バイト (len <= 8 は 0、len < 16 は末尾重複込み)
    uint32_t len;            // 0 = 空 (チャンネル名は必ず 1 文字以上なので衝突しない)
    uint32_t name_idx;       // names[] への添字 (出力と 25 文字以上の中間照合用)
    Stats months[MONTHS];
} Slot;

// スレッドごとの独立した集計状態。table は融合後 5.2 MB/スレッドで、旧設計の
// table 512 KB + stats 2.88 MB より大きい (このフットプリント増が吉と出るか本番で問う)。
typedef struct {
...
static_assert(sizeof(Stats) == 24, "サイズが変わったらライン跨ぎ率を再検討");
static_assert(sizeof(Slot) == 32 + 24 * MONTHS, "融合スロットのサイズ想定");
...
static void init_ctx(Ctx *c) {
    // len = 0 が空スロット。count_stamps / max / total は 0 初期化でよく、min_len だけは
    // 挿入時に UINT32_MAX にする。ファーストタッチも自スレッドで行う
...
HOT_INLINE Slot *find_slot(Ctx *c, const Row *r) {
    uint32_t len = r->channel_len;
    for (uint32_t slot = r->slot;; slot = (slot + 1) & (TABLE_SIZE - 1)) {
        Slot *e = &c->table[slot];
        if (e->len == 0) {  // 空スロット = 新規チャンネル
            uint32_t idx = (uint32_t)c->channel_count++;
            memcpy(c->names[idx], r->channel, len);
            c->names[idx][len] = '\0';
            e->first8 = r->first8;
            e->last8 = r->last8;
            e->mid8 = r->mid8;
            e->len = len;
            e->name_idx = idx;
            for (int m = 0; m < MONTHS; m++) e->months[m].min_len = UINT32_MAX;
            return e;
        }
        ...
        if (e->len == len && e->first8 == r->first8 && e->last8 == r->last8 &&
            (len <= 16 || (e->mid8 == r->mid8 &&
                           (len <= 24 || middle_equal(c->names[e->name_idx], r->channel, len))))) {
            return e;
        }
    }
}
...
static void update_row(Ctx *c, const Row *r) {
    update_stats(&find_slot(c, r)->months[month_of(r->ts8)], r);
}
...
static void run_range(Ctx *c, const char *p, const char *end) {
    ...
    while (p < end) {
        ...
        for (int i = 0; i < n; i++) {
            dst[i] = &find_slot(c, &rows[i])->months[month_of(rows[i].ts8)];
...
static void *worker(void *arg) {
    Ctx *c = arg;
    init_ctx(c);  // 表の初期化もファーストタッチも自スレッドで行う
...
static void merge_into_first(Ctx *dst, const Ctx *src) {
    for (uint32_t s = 0; s < TABLE_SIZE; s++) {
        const Slot *se = &src->table[s];
        if (se->len == 0) continue;
        Row r = {.channel = src->names[se->name_idx], .channel_len = se->len};
        fingerprint(&r, load8(r.channel));
        Slot *de = find_slot(dst, &r);
        for (int m = 0; m < MONTHS; m++) {
            const Stats *s2 = &se->months[m];
            if (s2->count_stamps == 0) continue;
            Stats *d = &de->months[m];
            if (s2->min_len < d->min_len) d->min_len = s2->min_len;
            if (s2->max_len > d->max_len) d->max_len = s2->max_len;
            d->total_len += s2->total_len;
            d->count_stamps += s2->count_stamps;
        }
    }
}
...
int main(int argc, char **argv) {
    ...
    for (uint32_t sl = 0; sl < TABLE_SIZE; sl++) {
        const Slot *e = &ctx[0].table[sl];
        if (e->len == 0) continue;
        for (int m = 0; m < MONTHS; m++) {
            const Stats *s = &e->months[m];
            uint32_t count = (uint32_t)(s->count_stamps >> 32);
            if (count == 0) continue;
            fprintf(out, "%s,2027-%02d=%" PRIu32 "/%.2f/%" PRIu32 "/%" PRIu32 "/%" PRIu32 "\n",
                    ctx[0].names[e->name_idx], m + 1, s->min_len, (double)s->total_len / (double)count,
