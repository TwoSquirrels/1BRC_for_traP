// 17-renumber のスレッド数を物理コア相当の 4 に固定した実験版。入力待ちが支配的に
// なった今、HT 兄弟が L2 (2 MB/コア) と入力帯域を分け合う損が演算並列度の得を上回って
// いないかを本番で測る (手元は HT の上積みゼロを観測済みだが本番とは別物)。
...
// 本番 (物理 4 コア × HT) で「スレッド半減 = 1 スレッドあたりの L2・入力帯域を倍化」の
// 損益を測る実験のため、上限を物理コア数相当の 4 に固定する
constexpr int THREAD_CAP = 4;

static int thread_count(void) {
    cpu_set_t set;
    if (sched_getaffinity(0, sizeof(set), &set) == 0) {
        int n = CPU_COUNT(&set);
        return n < 1 ? 1 : n > THREAD_CAP ? THREAD_CAP : n;
    }
    return THREAD_CAP;
}
