// 05-batch を土台にしたハッシュ表 L1 化版。05 からの差分:
//   - 開番地表のスロットを 24 B から 4 B (16 bit 指紋 + 16 bit ID) に圧縮し、
//     表全体を 64 KB (L1d の半分) に納める。プローブ時に読むのは指紋のみで、
//     指紋一致時だけ ID 直引きの照合配列 (first8/last8/len) と突き合わせる。
//     05 の結果 (プリフェッチで +3.3% 止まり) から「ハッシュコストの本丸は
//     レイテンシではなく表フットプリント」と仮説を立てた、その検証。
...
// プローブで読むのは 4 B のスロットだけ。指紋 0 は空きを表す (ハッシュ側で 0 を避ける)
typedef struct {
    uint16_t fingerprint;
    uint16_t id;
} Slot;

// 指紋一致時のみ参照する照合データ (ID 直引き)。挿入済みチャンネルの正確な同定に使う
typedef struct {
...
// hot な配列は 64 バイト境界に揃える (構造体先頭の整列だけでは中の配列は揃わない)。
// Slot は 4 B なので表は 64 KB に収まり、キャッシュライン跨ぎもゼロになった。
typedef struct {
    ...
    Key keys[MAX_CHANNELS];
    char names[MAX_CHANNELS][MAX_CHANNEL_LEN + 1];
    int channel_count;
} Ctx;
static_assert(sizeof(Slot) == 4, "表 = TABLE_SIZE × 4 B = 64 KB (L1d の半分) を維持する");
static_assert(sizeof(Slot[TABLE_SIZE]) == 64 * 1024, "");
static_assert(MAX_CHANNELS <= UINT16_MAX, "ID は 16 bit に収まる");

static Ctx ctx[MAX_THREADS];

static void init_ctx(Ctx *c) {
    memset(c->table, 0, sizeof(c->table));  // 指紋 0 = 空き
...
// ハッシュ値の上位 16 bit を指紋、下位をスロット位置に使う (独立に近い 2 つの縮約)。
// 指紋 0 は空きスロットの印なので 1 へ寄せる。
static void hash_channel(Row *r) {
    ...
    const uint64_t h = mix(r->first8 ^ (r->last8 * 0x9E37'79B9'7F4A'7C15ull) ^ r->channel_len);
    r->slot = (uint32_t)(h >> 32) & (TABLE_SIZE - 1);
    const uint16_t fp = (uint16_t)(h >> 48);
    r->fingerprint = fp == 0 ? 1 : fp;
}

static int channel_id(Ctx *c, const Row *r) {
    const uint32_t len = r->channel_len;
    for (uint32_t slot = r->slot;; slot = (slot + 1) & (TABLE_SIZE - 1)) {
        Slot *e = &c->table[slot];
        if (e->fingerprint == r->fingerprint) {
            const Key *k = &c->keys[e->id];
            if (k->len == len && k->first8 == r->first8 && k->last8 == r->last8 &&
                (len <= 16 || memcmp(c->names[e->id] + 8, r->channel + 8, len - 16) == 0)) {
                return e->id;
            }
            continue;  // 指紋衝突 (別チャンネル)。次のスロットへ
        }
        if (e->fingerprint == 0) {
            const int id = c->channel_count++;
            memcpy(c->names[id], r->channel, len);
            c->names[id][len] = '\0';
            c->keys[id] = (Key){.first8 = r->first8, .last8 = r->last8, .len = len};
            *e = (Slot){.fingerprint = r->fingerprint, .id = (uint16_t)id};
            return id;
        }
    }
}
