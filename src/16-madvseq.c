// 15-batch16 に madvise(MADV_SEQUENTIAL) を足した版 (差分は 1 行)。シーケンシャル走査を
// カーネルに宣言して先読み窓を倍増させ、ページキャッシュ限界運用の本番で入力の一部が
// 冷えていた場合のディスク読み戻しを太くする。温かければ中立、助言 API なので害はない。
// 手元 (常時温か) では原理的に測れないため本番を計測器にする。
...
int main(int argc, char **argv) {
    ...
    // シーケンシャル走査宣言でカーネルの先読み窓を倍増させる。ページキャッシュ限界運用の
    // 本番で一部が冷えていた場合のディスク読み戻しを太くする狙い (温かければ中立)
    madvise((void *)data, (size_t)st.st_size, MADV_SEQUENTIAL);
