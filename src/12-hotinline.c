// 11-simdscan のホットパス関数に always_inline を強制した版。11 からの差分は属性のみで
// アルゴリズムは不変。perf の関数一覧に channel_id / fingerprint / update_stats が独立
// シンボルとして現れて発覚: 11 は vpcmpeqb 追加で worker が肥大化した結果、コンパイラが
// ホットパスの展開をやめ、毎行 3〜4 回の call/ret + Row ポインタ渡しを払っていた。
