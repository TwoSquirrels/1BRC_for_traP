// 14-thp のバッチ幅を 8 → 16 に広げた版 (差分は BATCH 定数のみ)。プリフェッチ発行から
// 消費までの距離を倍にして、表スロット・集計先のロード待ちをより深く覆う狙い。
// 手元 A/B は 10 世代でノイズ混じりの判定不能だったグレー項目で、「一貫した負けで
// なければ本番で決着」の基準に従い本番を計測器にする。13 で照合が軽くなり (mid8)、
// バッチ前段の滞留が減った今なら損益が動いている可能性もある。
