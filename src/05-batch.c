...
// Slot 24 B / Stats 20 B は 64 の約数でないため、それでも要素の 25% はライン跨ぎになる
// (32 B へのパディングはフットプリント増との取引なので計測してから)。
typedef struct {
...
static_assert(sizeof(Slot) == 24 && sizeof(Stats) == 20, "サイズが変わったらライン跨ぎ率を再検討");
