// 17-renumber に入力ストリームの明示プリフェッチ (512 B ≈ 20 行先) を足した版。
// 17 の perf で入力行の初回ロード (名前スキャンの vpcmpeqb のメモリオペランド) に
// 4 割のサンプルが集中し、HW プリフェッチャだけでは入力レイテンシを隠し切れていない
// 疑いが出たため。手元 A/B はグレー (−0.6 / +9.0 / −4.6) につき本番で決着させる。
...
static void run_range(Ctx *c, const char *p, const char *end) {
    ...
    while (p < end) {
        ...
        int n = 0;
        while (n < BATCH && p < end) {
            // 入力ストリームの先読み。perf で入力行の初回ロード (名前スキャンの vpcmpeqb)
            // に 4 割のサンプルが集中しており、HW プリフェッチャ任せでは届いていない。
            // 512 B 先 ≈ 20 行先で、バッチ 1 周ぶんの距離を確保する
