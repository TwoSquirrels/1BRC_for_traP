// 07-fusedscan に 08-multistream の演算側改善だけを移植した切り分け版。08 の大幅悪化
// (本番 8.136 秒) の主犯がマルチストリーム (メモリアクセス形状) なのか演算側の変更なのかを
// 分離するため、行の走査構造は 07 のまま (単一ストリーム + 8 行バッチ + プリフェッチ) とし、
// 以下の 2 点のみ取り込む:
