// 10-statprefetch の名前スキャンを SIMD 化した版。10 からの差分:
//   - チャンネル名の ',' 探索を「8 B ワード 2 本のゼロバイト検出 + cmov 選択」から
//     vpcmpeqb 1 発 + tzcnt に置き換える。提出ターゲット (AVX-512BW) は 64 B 一括で
//     63 文字まで、手元検証用 (AVX2) は 32 B 一括で 31 文字までを分岐なしで確定し、
//     それ以上だけ 8 B ループへ落ちる (実データの名前は 3〜32 文字、保証上限 104)。
//   - 先読みロードが 64 B に広がったため、写像末尾の安全余白を「最終行のみバッファ」から
//     「末尾 160 B 以内に始まる行を全部バッファ」に拡張 (160 = 最悪読み位置の上界 +
//     行長上限からの余裕。詳細は main のコメント)。
// 提出物 (AVX-512 経路) は手元 CPU で実行できない初のケース。AVX2/AVX-512 の差分を
// name_len() 内の数行に閉じ込め、正しさは手元 AVX2 + 本番 verify の二段で確認する。
...
static void update_stats(Stats *s, const Row *r) {
