...
// 提出バイナリの musl では memcmp がスカラーループに落ちるため自前比較で置き換える。
...
static void run_range(Ctx *c, const char *p, const char *end) {
    Row rows[BATCH];
    Stats *dst[BATCH];
    while (p < end) {
...
static int desc_u64(const void *a, const void *b) {
    uint64_t x = *(const uint64_t *)a, y = *(const uint64_t *)b;
    return x < y ? 1 : x > y ? -1 : 0;
}

// 第 1 相で数えた出現頻度の降順に ID を振り直し、stats / names / table を並べ替える。
// 頻出チャンネルの集計行が stats 先頭の数 KB に密集し、L1/L2 に安定して住み着く。
// サンプリングが外れても正しさは不変で、配置が最適でなくなるだけ (実行時学習の安全性)
static void renumber(Ctx *c) {
    int n = c->channel_count;
    if (n <= 1) return;
    static_assert(TABLE_SIZE <= (1 << 14), "ID を下位 14 bit に詰める前提");
    uint64_t order[MAX_CHANNELS];
    for (int ch = 0; ch < n; ch++) {
        uint64_t total = 0;
        for (int m = 0; m < MONTHS; m++) total += c->stats[ch][m].count_stamps >> 32;
        order[ch] = (total << 14) | (uint64_t)ch;
    }
    qsort(order, (size_t)n, sizeof(uint64_t), desc_u64);
    int perm[MAX_CHANNELS];  // 旧 ID → 新 ID
    for (int rank = 0; rank < n; rank++) perm[order[rank] & 0x3FFF] = rank;

    // renumber は配置最適化にすぎないため、一時領域を確保できなければ元の ID のまま続行する
    Stats(*stats_tmp)[MONTHS] = malloc((size_t)n * sizeof *stats_tmp);
    if (stats_tmp == nullptr) return;
    char(*names_tmp)[MAX_CHANNEL_LEN + 1] = malloc((size_t)n * sizeof *names_tmp);
    if (names_tmp == nullptr) {
        free(stats_tmp);
        return;
    }
    for (uint32_t s = 0; s < TABLE_SIZE; s++) {
        if (c->table[s].id >= 0) c->table[s].id = perm[c->table[s].id];
    }
    memcpy(stats_tmp, c->stats, (size_t)n * sizeof *stats_tmp);
    memcpy(names_tmp, c->names, (size_t)n * sizeof *names_tmp);
    for (int old = 0; old < n; old++) {
        memcpy(c->stats[perm[old]], stats_tmp[old], sizeof c->stats[0]);
        memcpy(c->names[perm[old]], names_tmp[old], sizeof c->names[0]);
    }
    free(stats_tmp);
    free(names_tmp);
}
...
static void *worker(void *arg) {
    ...
    // 第 1 相: 担当範囲の先頭 1/128 (本番なら約 25 MB ≈ 100 万行) を普通に処理しながら
    // 頻度を学び、ID を頻度順に振り直してから残りを本走する
    const char *sample_end = c->begin + (c->end - c->begin) / 128;
    if (sample_end > c->begin && sample_end < c->end) {
        const char *nl = memchr(sample_end, '\n', (size_t)(c->end - sample_end));
        sample_end = nl != nullptr ? nl + 1 : c->end;
        run_range(c, c->begin, sample_end);
        renumber(c);
        run_range(c, sample_end, c->end);
    } else {
        run_range(c, c->begin, c->end);
    }
    return nullptr;
}
...
// 出力の自前フォーマット。集計行 (最大 12 万行) を巨大バッファに詰めて write 1 発で出す。
// 律速ではないが、平均値の %.2f が呼ぶ __printf_fp は 1 行ごとに重いので、整数は自前変換、
// 平均は printf 互換丸め (reference.zig の writeAvg の移植) に置き換えて丸ごと外す。
...
// total / count を printf("%.2f") と厳密に同じ最近接偶数丸めで小数第 2 位まで書く。
// double の仮数・指数を取り出し、mantissa * 100 * 2^exp を 1/100 単位へ丸める。
static char *put_avg(char *p, uint32_t total, uint32_t count) {
    double avg = (double)total / (double)count;
    uint64_t bits;
    memcpy(&bits, &avg, sizeof bits);
    int biased_exp = (int)((bits >> 52) & 0x7ff);
    uint64_t fraction = bits & (((uint64_t)1 << 52) - 1);
    unsigned __int128 mantissa = biased_exp == 0 ? fraction : (fraction | ((uint64_t)1 << 52));
    int exp = biased_exp == 0 ? -1074 : biased_exp - 1075;

    unsigned __int128 scaled = mantissa * 100;
    unsigned __int128 hundredths;
    if (exp >= 0) {
        hundredths = scaled << exp;
    } else if (-exp > 126) {
        hundredths = 0;  // 集計上ありえない極小値だが 0.00 に丸める
    } else {
        int shift = -exp;
        unsigned __int128 divisor = (unsigned __int128)1 << shift;
        unsigned __int128 quotient = scaled >> shift;
        unsigned __int128 remainder = scaled & (divisor - 1);
        hundredths = quotient;
        if (remainder * 2 > divisor || (remainder * 2 == divisor && (quotient & 1))) hundredths += 1;
    }
    p = put_u64(p, (uint64_t)(hundredths / 100));
    uint32_t frac = (uint32_t)(hundredths % 100);
    *p++ = '.';
    *p++ = (char)('0' + frac / 10);
    *p++ = (char)('0' + frac % 10);
    return p;
...
int main(int argc, char **argv) {
    ...
    for (int t = 1; t < threads; t++) {
        if (pthread_create(&tid[t], nullptr, worker, &ctx[t]) == 0) {
            created[t] = true;
        } else {
            // 独立した担当範囲なので、生成失敗時は main スレッドで処理すれば正しさを保てる
            worker(&ctx[t]);
        }
    }
    worker(&ctx[0]);
    for (int t = 1; t < threads; t++) {
        ...
        int err = pthread_join(tid[t], nullptr);
        if (err != 0) {
    ...
    // 1 行の上界: 名前 104 + ",2027-MM=" 9 + 整数 4 つ (各 10) + 平均 (整数部 10 + ".dd" 3)
    // + 区切り 4 + '\n' = 174 < 192。全行ぶんを 1 バッファに詰める
    constexpr size_t LINE_CAP = 192;
    size_t cap = (size_t)ctx[0].channel_count * MONTHS * LINE_CAP;
    char *buf = malloc(cap);
    if (buf == nullptr) {
        perror("malloc");
        return 1;
    ...
    for (int c = 0; c < ctx[0].channel_count; c++) {
        ...
        for (int m = 0; m < MONTHS; m++) {
            ...
            *p++ = (char)('0' + (m + 1) % 10);
            *p++ = '=';
            p = put_u64(p, s->min_len);
            *p++ = '/';
            p = put_avg(p, s->total_len, count);
            *p++ = '/';
            p = put_u64(p, s->max_len);
            *p++ = '/';
            p = put_u64(p, count);
            *p++ = '/';
            p = put_u64(p, (uint32_t)s->count_stamps);
            *p++ = '\n';
        }
    }
    for (char *w = buf; w < p;) {
        ssize_t n = write(ofd, w, (size_t)(p - w));
        if (n > 0) {
            w += n;
