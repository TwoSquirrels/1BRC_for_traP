// 12-hotinline の指紋を 3 ワードに拡張した版。12 のインライン化後の perf で、17 文字以上の
// 照合が引く names 配列 (1 MB・ランダムアクセス・プリフェッチなし) のロードが最大の単一
// ストール (worker 内 13.5%) と判明したため、Slot に mid8 (名前の [8, 16) バイト) を追加。
// (first8, last8, len) で 16 文字以下、+ mid8 で 24 文字以下の照合が指紋のみで完結し、
// names を踏む行が 8.4% → 1.5% (25 文字以上のみ) に減る。Slot は 24 → 32 B になり、
// ライン跨ぎスロット (24 B 時代は 25%) も消える。単純パディングの 32 B 化は A/B 中立
// だったが、同じ空間に照合データを載せたら勝った (フットプリント増 33% < names 参照減)。
